...
fn memory_is_secret(memory: &MemoryKind) -> bool {
    match memory {
        MemoryKind::Import {ty, ..}
        | MemoryKind::Normal (ty) => {
            match ty {
                MemoryType::B32 {secret, ..}
                | MemoryType::B64 {secret, ..} => {
                    *secret
                }
            }
        },
...
fn memory_is_imported(memory: &MemoryKind) -> bool {
    match memory {
        MemoryKind::Import {..} => {
            true
        },
        MemoryKind::Normal (..) | MemoryKind::Inline {..} => {
            false
        }
    }
}

fn get_func_type<'a>(func: &'a Func, types: &'a Vec<&Type>) -> &'a FunctionType<'a> {
    match func.ty.index {
        Some(idx) => {
            match idx {
                Index::Num(i, ..) => {
                    match &types[i as usize].def {
                        TypeDef::Func(ty) => ty,
                        _ => panic!("invalid function index")
                    }
                },
                Index::Id(..) => panic!("indices should be resolved at this point")
            }
        }
        None => {
            match &func.ty.inline {
                Some(ty) => ty,
                None => panic!("function has neither type index nor inline type"),
            }
        }
    }
}

fn valty_is_secret(ty: ValType) -> bool {
    match ty {
        ValType::S32 | ValType::S64 => true,
        _ => false
    }
}

fn gen_ct_wasm_func_sig(func: &Func, types: &Vec<&Type>) -> CtWasmFunctionSig {
    let ty = get_func_type(func, types);
    let trusted = ty.trusted;
    let mut param_secrecy = Vec::with_capacity(ty.params.len());
    let mut return_secrecy = Vec::with_capacity(ty.results.len());

    for i in 0..ty.params.len() {
        param_secrecy.push(valty_is_secret(ty.params[i].2));
    }

    for i in 0..ty.results.len() {
        return_secrecy.push(valty_is_secret(ty.results[i]));
    }
...
fn gen_ct_annot<'a>(
    ...
    let mut global_secrecies = Vec::with_capacity(globals.len());
    let mut memory_sigs = Vec::with_capacity(memories.len());
    let mut function_sigs = Vec::with_capacity(funcs.len());

    for i in 0..globals.len() {
        let global = globals[i];
        global_secrecies.push(match global.ty.ty {
            ValType::S32 | ValType::S64 => true,
            _ => false
        })
    }
    ...
    for i in 0..imports.len() {
        ...
        match import.item.kind {
            ItemKind::Memory(ty) => {
                memory_sigs.push(CtWasmMemorySig {
                    imported: true,
                    secret: match ty {
                        MemoryType::B32 {secret, ..}
                        | MemoryType::B64 {secret, ..} => {
    ...
    for i in 0..memories.len() {
        ...
        memory_sigs.push(CtWasmMemorySig {
            imported: memory_is_imported(&memory.kind),
            secret: memory_is_secret(&memory.kind)
        })
    }

    for i in 0..funcs.len() {
        let func = funcs[i];
        function_sigs.push(gen_ct_wasm_func_sig(func, types))
    }

    let json = json!({
        "globalsSecrecy": global_secrecies,
        "memories": memory_sigs,
...
fn encode_fields(
    ...
    let ct_annot = gen_ct_annot(&funcs, &globals, &memories, &imports, &types);
    annot.push(ct_annot.as_bytes());
...
impl<'a> Encode for ValType<'a> {
    fn encode(&self, e: &mut Vec<u8>) {
        match self {
            ValType::I32 | ValType::S32 => e.push(0x7f),
            ValType::I64 | ValType::S64 => e.push(0x7e),
